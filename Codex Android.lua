...
local unavailable = function(fake)
	return function(...)
		cloneref(game:GetService("StarterGui")):SetCore("SendNotification", {
			Title = "Codex Android by Duc Designer",
			Text = "Tính năng này không thể được sử dụng trên phiên bản apk hiện tại"
		});
		...
		changelog = {
			{
				...
				data = {
					"Dữ liệu nhật ký thay đổi không thể tải xuống"
				}
			}
		}
	};
	...
	local function createUI(directory: Instance): ScreenGui
		ui = instanceUtils:Create("ScreenGui", {
			...
			textLabel({
				...
				Text = "Codex Android by Duc Designer",
				TextSize = 24
			}),
			textLabel({
				AnchorPoint = Vector2.new(0.5, 0.5),
